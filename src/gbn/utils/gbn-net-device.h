 ...
 */
#ifndef GBN_NET_DEVICE_H
#define GBN_NET_DEVICE_H
 ...
 * 
 * The device can be installed on a node through the GbnNetDeviceHelper.
 ...
 */
class GbnNetDevice : public NetDevice
{
public:
  /**
  ...
  /**
   * Receive a packet from a connected GbnChannel.  The 
   * GbnNetDevice receives packets from its connected channel
   ...
   *
   */
  void SetChannel (Ptr<GbnChannel> channel);

  /**
   * Attach a queue to the GbnNetDevice.
   *
  ...
  /**
   * Attach a receive ErrorModel to the GbnNetDevice.
   *
   * The GbnNetDevice may optionally include an ErrorModel in
...
private:
  Ptr<GbnChannel> m_channel; //!< the channel the device is connected to
   ...
   * due to the error model being active.  Although GbnNetDevice doesn't 
