 ...
 */
class GbnReceiverHelper
{
public:
  /**
   * Create GbnReceiverHelper which will make life easier for people trying
  ...
  /**
   * Create a GbnReceiverApplication on the specified Node.
   *
  ...
  /**
   * Create a GbnReceiverApplication on specified node
   *
...
private:
  /**
   * Install an ns3::GbnReceiver on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an GbnReceiver will be installed.
...
public:
  ...
  /**
   * Given a pointer to a GbnSender application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be GbnSender).
   ...
   *
   * \param node The Ptr<Node> on which to create the GbnSenderApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the GbnSenderApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::GbnSender on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an GbnSender will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
